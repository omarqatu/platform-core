...
**Build:**
- A .NET solution with separate projects: `Core` (the core), `Modules.Subscriptions` (the minimal module — T5), `Api`, `Migrations`, `Conformance` (the tests — independent, referencing `Core` only via HTTP and SQL), and (1.1, added in T1) `Core.WhiteBoxTests` for `[W]` tests only — it references `Core` directly and is not run when evaluating another implementation (Section 6).
...
| T0.3 | `Api`'s configuration and image contain no `migrator` connection string — (1.1) in two layers: matching connection-string shapes (case-insensitive, with whitespace, including UTF-16 encoding inside DLLs and environment variables), and a literal search for the `migrator` password value, which CI generates randomly on every run | [W] |
...
- (1.1) A temporary test table `t1_probe` carrying the verbatim `tenant_isolation` text (in its v1.10 `NULLIF` form), created and dropped within the run and never entering the migrations — T1.2, T1.3, and T1.6 are mechanism proofs on it, re-proven against a real table after T2.
...
| T1.6 | (1.1) Test 27 — the first-template part: SET LOCAL, then COMMIT or ROLLBACK, then DISCARD ALL, then a query with no context on the same connection → zero rows **with no error** | [B] |
