...
**Version:** 1.9 — **Frozen release**
**Date:** 2026-09-23
**Review history:** Seven architecture reviews + two model reviews (through 1.6), then a **requirement-driven change** in 1.7, then an **eighth external review** that settled 1.8, then a **ninth review** that settled 1.9. The full changelog is in the appendices.
...
CREATE POLICY client_scope ON <t>
  ...
  USING (
    COALESCE((SELECT current_setting('app.scope_all', true)::boolean), false)
    OR EXISTS (
      SELECT 1 FROM scope_assignments sa
      WHERE sa.tenant_id     = <t>.tenant_id
        AND sa.scope_ref_id  = <t>.scope_ref_id
        AND sa.membership_id = (SELECT current_setting('app.membership_id', true)::uuid)
        AND sa.active))
...
| Fail-safe | Without setting app.tenant_id → zero rows (the last-resort net — 3.5). And on the second axis: without `app.scope_all` or `app.membership_id` → zero rows as well (3.1) |
...
CREATE POLICY person_self ON persons
  ...
  USING (EXISTS (
    ...
    WHERE u.person_id = persons.id
      AND u.id = (SELECT current_setting('app.user_id', true)::uuid)));
...
-- A matching policy on users via memberships.
...
CREATE POLICY membership_scope_read ON membership_scope
  ...
  USING (
    ...
    AND (COALESCE((SELECT current_setting('app.can_manage_scope', true)::boolean), false)
         OR membership_id = (SELECT current_setting('app.membership_id', true)::uuid)));
...
CREATE POLICY membership_scope_admin_update ON membership_scope
  ...
  USING (
    tenant_id = (SELECT current_setting('app.tenant_id', true)::uuid)
    AND COALESCE((SELECT current_setting('app.can_manage_scope', true)::boolean), false)
    AND membership_id <> (SELECT current_setting('app.membership_id', true)::uuid))
  WITH CHECK ( … same expression … );
...
CREATE POLICY audit_read ON audit_log
  ...
  USING (
    tenant_id = (SELECT current_setting('app.tenant_id', true)::uuid)
    AND COALESCE((SELECT current_setting('app.scope_all', true)::boolean), false));
```
...
**1. The leanest proof-of-concept spec,** as Codex tasks, ordered as: the login and tenant-selection path first — **including the second-axis resolution in its mandatory three-step order (3.5/6), with its Test 26 run before anything after it** — then a full invite-and-accept cycle, then the unified view with cursor merging, then adversarial isolation (the twenty-six tests + the ten CI checks), then identity, bootstrap, and the auth layer, then the minimal subscriptions screen.
...
## Appendix A — Changelog from 1.8 to 1.9
...
## Appendix B — Changelog from 1.7 to 1.8
...
## Appendix C — Changelog from 1.6 to 1.7
...
## Appendix D — Changelog from 1.5 to 1.6
