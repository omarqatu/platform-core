...
**Verdict: EXCEEDED** — p95 ratio 1.574 (both 9.451 ms / alone 6.005 ms; the limit is 1.5).
...
|---|---|---|---|---|---|---|---|---|---|
| both policies (tenant_isolation + client_scope) | query | 2000 | 1.183 | 4.505 | 7.817 | 9.451 | 15.270 | 43.923 | 4.987 |
| both policies (tenant_isolation + client_scope) | transaction | 2000 | 5.948 | 36.470 | 48.167 | 53.766 | 95.657 | 137.468 | 37.272 |
| tenant_isolation alone (client_scope neutralized) | query | 2000 | 0.476 | 2.488 | 4.766 | 6.005 | 9.195 | 21.436 | 2.864 |
| tenant_isolation alone (client_scope neutralized) | transaction | 2000 | 4.273 | 24.668 | 35.258 | 38.795 | 46.922 | 82.039 | 25.578 |

Transaction p95 ratio (for information, not the criterion): 1.386.
...
Limit  (cost=0.33..318.02 rows=50 width=46) (actual time=0.057..0.320 rows=50.00 loops=1)
  ...
  ->  Index Scan using subscriptions_pkey on subscriptions s  (cost=0.29..31807.85 rows=5006 width=46) (actual time=0.056..0.315 rows=50.00 loops=1)
        ...
        Buffers: shared hit=23
        SubPlan 11
          ->  Seq Scan on scope_assignments sa  (cost=0.08..1.48 rows=3 width=32) (actual time=0.018..0.021 rows=20.00 loops=1)
                ...
                InitPlan 7
                  ->  Result  (cost=0.00..0.02 rows=1 width=16) (actual time=0.000..0.001 rows=1.00 loops=1)
                InitPlan 8
...
Limit  (cost=0.31..7.18 rows=50 width=46) (actual time=0.016..0.027 rows=50.00 loops=1)
  Buffers: shared hit=3
  InitPlan 1
    ->  Result  (cost=0.00..0.02 rows=1 width=16) (actual time=0.002..0.002 rows=1.00 loops=1)
  ->  Index Scan using subscriptions_pkey on subscriptions s  (cost=0.29..917.74 rows=6675 width=46) (actual time=0.016..0.024 rows=50.00 loops=1)
